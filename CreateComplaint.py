...
class CreateComplaint:
    ...
    def add_header_section(self):
        ...
        self.welcome_label = Label(self.header_frame, text="Enter the Details of Complaint",
    ...
    def setup_homepage(self):
        ...
        self.lab = Label(self.body_frame, width=50, text="Enter below details to create a new complaint")
        ...
        self.e_Date = Entry(self.body_frame, width=30, fg='black', bg='white' )
        self.e_cd = Entry(self.body_frame, width=35, fg='black', bg='white' )
        ...
        Label(self.body_frame, width=20, text="Enter Your Name: ").grid(row=2, column=1, padx=10, pady=10)
        Label(self.body_frame, width=20, text="Enter Today's Date ").grid(row=3, column=1, padx=10, pady=10)
        Label(self.body_frame, width=20, text="Complaint Details: ").grid(row=4, column=1, padx=10, pady=10)
        ...
        self.e_Date.grid(row=3, column=2, padx=10, pady=10)


        self.e_contact.grid(row=5, column=2, padx=10, pady=10)
        self.e_email.grid(row=6, column=2, padx=10, pady=10)
        self.e_cd.grid(row=4, column=2, padx=10, pady=10)
        self.e_username.focus_set()
        self.b_register = Button(self.body_frame, text="Complaint!", height=2, width=10,
    ...
    def register_user(self):
        ...
        complaint = self.e_cd.get()
        if (name == "" or contact == "" or email == "" or date == "" or complaint ==""):
            messagebox.showwarning("Mandatory fields", "Please fill all the fields")
            self.registration_window.tkraise()

        else:
            query = "insert into Complaint (NameOfPerson ,ComplaintIssueDate ,ComplaintDescription,Contact,Email) values (%s,%s,%s,%s,%s)"
            args = (name,date, complaint, contact, email)
            DatabaseHelper.execute_query(query, args)
            messagebox.showinfo("Success", "Complaint has been generated /n which will be resloved ASAP!")
